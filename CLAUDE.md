...
- **`content.js`** — Content script injected into YouTube pages. Contains `hideContent()` for DOM hiding. A `MutationObserver` on `document.body` re-runs on DOM changes to handle YouTube's dynamic SPA navigation. Also listens to `chrome.storage.onChanged` to react to preference toggles in real-time.
- **`popup.js` + `popup.html`** — Extension popup UI with checkboxes (Hide Shorts, Hide Playables). Preferences are persisted to `chrome.storage.sync` on every toggle. Both default to `true` (enabled).
